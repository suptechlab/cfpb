...
def mk_working(myST):
	...
	mySQL = mySQL + "CREATE TABLE " + sch + ".uauc_" + myST + " ( "
	mySQL = mySQL + "gid serial not null, " 
	mySQL = mySQL + "uace10 character varying(5), "
	mySQL = mySQL + "name10 character varying(100), "
	mySQL = mySQL + "geom geometry(Polygon, 4326)); "
	mySQL = mySQL + "DROP TABLE IF EXISTS " + sch + ".working; COMMIT; "
	mySQL = mySQL + "CREATE TABLE " + sch + ".working AS "
	...
	mySQL = mySQL + "INSERT INTO " + sch + ".uauc_" + myST + " (uace10, name10, geom) "
	mySQL = mySQL + "select uace10, name10, (st_dump(geom)).geom as geom "
	mySQL = mySQL + "from " + sch + ".working; "
