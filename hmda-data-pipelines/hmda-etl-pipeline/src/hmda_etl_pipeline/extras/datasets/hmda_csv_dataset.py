"""``HmdaCSVDataset`` loads/saves data from/to a CSV file using an underlying
...
class HmdaCSVDataset(AbstractVersionedDataset):
    ...
    def _save(self, data: pd.DataFrame) -> None:
        latest_save_path = get_filepath_str(self._get_save_path(), self._protocol)
        ...
        with self._fs.open(
            latest_save_path, **self._fs_open_args_save
        ) as fs_file_latest:
            data.to_csv(path_or_buf=fs_file_latest, **self._save_args)

        # Save to dated folder
        with self._fs.open(dated_save_path, **self._fs_open_args_save) as fs_file_dated:
            data.to_csv(path_or_buf=fs_file_dated, **self._save_args)
