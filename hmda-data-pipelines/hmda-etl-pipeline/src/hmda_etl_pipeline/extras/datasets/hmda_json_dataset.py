"""``HmdaJSONDataset`` loads/saves data from/to a JSON file using an underlying
...
class HmdaJSONDataset(AbstractVersionedDataset[pd.DataFrame, pd.DataFrame]):
    ...
    def _save(self, data: pd.DataFrame) -> None:
        ...
        # Get the json string for the data
        json_data_str = data.to_json(**self._save_args)

        # Fix formatting in the json string.
        # First, remove the escaped slashes on dates
        # Second, remove the {"0": and end } from the json file, which are
        # there because the data is from a single row dataframe
        fixed_json_data_str = json_data_str.replace("\\/", "/")[5:-1]

        # Put fixed json data string in byte buffer
        buf = BytesIO(bytes(fixed_json_data_str, "ascii"))
